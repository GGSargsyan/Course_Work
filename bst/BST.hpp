...
private:
  ...
  /** recursive helper method to help find the
   *height of BST*/
...
std::pair<BSTIterator<Data>, bool> BST<Data>::insert(const Data& item) {
  //Here we check if root exists, if not insert as root and return iterator bool pair
  if (!root) {
  ...
  //Check to make sure item is not duplicate
   if(find(item) != nullptr)
     return std::pair<BSTIterator<Data>, bool>(BSTIterator<Data>(find(item)), false);

  BSTNode<Data>* curr = root;
  //Traverse tree in order to find proper place to insert
...
BSTIterator<Data> BST<Data>::find(const Data& item) const
{
  //Start at root
  BSTNode<Data>* curr = root;
  //While node exists, traverse through tree and see if exists
  while (curr) {
    if (curr->data < item) {
      curr = curr->right;
    }
...
/**Recursive helper function to find the max
 * number of edges from root to leaf nodes*/
template <typename Data>
int BST<Data>::heightHelper(BSTNode<Data>* n) const 
{
  if(n == NULL)
    return 0;
  //if tree is only root return 0
  if(!n->left && !n->right)
    return 0;
  //Recursive call, return 1 for root + max(height left subtree, height right)
...
void BST<Data>::deleteAll(BSTNode<Data>* n)
{
  if(!n)
    return;
  //recursive calls for left, right subtree
  if(n->left)
    deleteAll(n->left);
